...
void test("stripe webhook accepts signatures generated with a rotated fallback secret", async () => {
  const config = createTestApiConfig({
    STRIPE_WEBHOOK_SECRET: "whsec_current_signature",
    STRIPE_WEBHOOK_SECRET_FALLBACKS: "whsec_previous_signature"
  });
  const stripe = new Stripe(config.STRIPE_SECRET_KEY, {
    apiVersion: STRIPE_API_VERSION
  });
  ...
  const signature = stripe.webhooks.generateTestHeaderString({
    payload,
    secret: "whsec_previous_signature"
  });
  const billingEvents = createBillingEventStubs();
  ...
  try {
    ...
    assert.equal(body.received, true);
    assert.equal(
      billingEvents.get("evt_rotated_signature")?.status,
      BillingEventStatus.processed
    );
  } finally {
    for (const restore of billingEvents.restores.reverse()) {
