...
import { createAccessToken, sha256 } from "../src/modules/auth/crypto.js";
...
void test("setAuthCookies hardens auth cookies with Strict sameSite, secure transport and priority", () => {
  const config = {
    ...createTestApiConfig(),
  ...
  assert.equal(sessionCookie.options.priority, "high");
  ...
  assert.equal(refreshCookie.options.priority, "high");
  ...
  assert.equal(csrfCookie.options.priority, "medium");
...
void test("authenticateRequest accepts a rotated fallback session secret while verifying claims", async () => {
  const config = createTestApiConfig({
    SESSION_SECRET: "session-current-secret",
    SESSION_SECRET_FALLBACKS: "session-legacy-secret"
  });
  const sessionToken = createAccessToken({
    organizationId: "org_1",
    role: Role.ADMIN,
    secret: "session-legacy-secret",
  ...
  const restores = [
    ...
    stubMethod(prisma.session, "update", () => Promise.resolve({ id: "session_rotated" }))
  ];
  ...
  try {
    ...
    assert.equal(authenticated.role, Role.ADMIN);
    assert.equal(authenticated.sessionId, "session_rotated");
    assert.equal(authenticated.tenantId, "tenant_1");
  } finally {
    for (const restore of restores.reverse()) {
      restore();
    }
