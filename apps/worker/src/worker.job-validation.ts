...
export function validateLegacyTaskJob(input: {
  ...
  const signatureCandidates = input.tenantSecret
    ? [signPayload(signedPayload, input.tenantSecret)]
    : input.fallbackSecrets.map((secret) => signPayload(signedPayload, secret));

  if (
    input.payload.signature !== "unsigned" &&
    !signatureCandidates.includes(input.payload.signature)
  ) {
