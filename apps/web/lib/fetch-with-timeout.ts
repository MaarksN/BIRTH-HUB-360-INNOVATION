...
function createTimeoutError(timeoutMs: number, timeoutMessage?: string): Error {
  const error = new Error(timeoutMessage ?? `Request exceeded the ${timeoutMs}ms timeout budget.`);
...
export async function fetchWithTimeout(
  ...
  const { signal, timeoutMessage, timeoutMs = DEFAULT_FETCH_TIMEOUT_MS, ...requestInit } = init;
