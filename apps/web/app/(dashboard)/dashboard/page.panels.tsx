...
function formatIntegrationDirection(locale: SupportedLocale, direction: string): string {
  if (locale === "pt-BR") {
    return direction === "inbound" ? "entrada" : "saida";
  }

  return direction === "inbound" ? "inbound" : "outbound";
}

function formatConnectorHealthStatus(locale: SupportedLocale, status: string | null): string {
  if (!status) {
    return locale === "pt-BR" ? "sem check" : "not checked";
  }

  if (locale === "pt-BR") {
    return status === "healthy" ? "saudavel" : "critico";
  }

  return status === "healthy" ? "healthy" : "unhealthy";
}

function formatConnectorImplementationStage(
  locale: SupportedLocale,
  stage: "client_only" | "implemented" | "planned"
): string {
  const labels = {
    client_only: locale === "pt-BR" ? "somente client" : "client only",
    implemented: locale === "pt-BR" ? "implementado" : "implemented",
...
export function DashboardIntegrationActivitySection(props: {
  data: DashboardHomeData;
  locale: SupportedLocale;
}) {
  const title = props.locale === "pt-BR" ? "Atividade de integracoes" : "Integration activity";
  const description =
    props.locale === "pt-BR"
      ? "Eventos recebidos, acoes executadas e retornos recentes dos conectores."
      : "Recent connector events, executed deliveries, and provider responses.";
  const emptyTitle =
    props.locale === "pt-BR" ? "Nenhuma atividade registrada" : "No activity recorded";
  const emptyDescription =
    props.locale === "pt-BR"
      ? "Quando o primeiro evento de conector for processado, ele aparece aqui."
      : "The first processed connector event will land here.";
  ...
  return (
    <section className="panel">
      <div className="dashboard-panel__header">
        <div className="dashboard-panel__copy">
          <h2>{title}</h2>
          <p className="dashboard-muted">{description}</p>
        </div>
        <Link href="/settings/developers/webhooks">
          {props.locale === "pt-BR" ? "Abrir webhooks" : "Open webhooks"}
        </Link>
      </div>

      {props.data.integrationActivity.connectors.length > 0 ? (
        <div className="dashboard-card-list">
          {props.data.integrationActivity.connectors.map((connector) => (
            <article className="dashboard-record-card" key={connector.accountId}>
              <div className="dashboard-card__header">
                <strong>{connector.provider}</strong>
                {connector.runtimeImplemented ? (
                  <span className="status-pill">
                    {formatConnectorHealthStatus(props.locale, connector.healthStatus)}
                  </span>
                ) : (
                  <span className="status-pill">
                    {formatConnectorImplementationStage(props.locale, connector.implementationStage)}
                  </span>
                )}
              </div>
              {connector.runtimeImplemented ? (
                <>
                  <span className="dashboard-record-card__meta">
                    {props.locale === "pt-BR" ? "status" : "status"} · {connector.status}
                  </span>
                  <span className="dashboard-record-card__meta">
                    {connector.lastExecutionAt
                      ? `${props.locale === "pt-BR" ? "ultima execucao" : "last execution"} · ${formatDateTime(props.locale, connector.lastExecutionAt, {
                          dateStyle: "medium",
                          timeStyle: "short"
                        })}`
                      : props.locale === "pt-BR"
                  ...
                  {connector.lastExecutionStatus ? (
                    <span className="dashboard-record-card__meta">
                      {props.locale === "pt-BR" ? "resultado" : "result"} · {connector.lastExecutionStatus}
                    </span>
                  ) : null}
                  {connector.lastError ? (
                    <span className="dashboard-record-card__meta">{connector.lastError}</span>
                  ) : null}
                </>
              ) : null}
            </article>
      ...
      {props.data.integrationActivity.items.length === 0 ? (
        <ProductEmptyState description={emptyDescription} title={emptyTitle} />
      ) : (
        <div className="dashboard-card-list">
          {props.data.integrationActivity.items.slice(0, 5).map((item) => (
            <article className="dashboard-record-card" key={item.id}>
              <div className="dashboard-card__header">
                ...
                <span className="status-pill">
                  {item.responseStatus ?? "pendente"}
                </span>
              </div>
              <span className="dashboard-record-card__meta">
                {formatIntegrationDirection(props.locale, item.direction)} ·{" "}
                {formatDateTime(props.locale, item.createdAt, {
                  dateStyle: "medium",
                  timeStyle: "short"
                })}
              </span>
