...
export const apiEnvSchema = z.object({
  ...
  OMIE_BASE_URL: urlString.default("https://app.omie.com.br/api/v1"),
...
export type ApiConfig = z.infer<typeof apiEnvSchema> & {
  ...
  jobHmacSecretCandidates: string[];
  secretCatalog: {
    authMfaEncryption: ReturnType<typeof describeResolvedSecret>;
    googleClient: ReturnType<typeof describeResolvedSecret> | null;
    hubspotClient: ReturnType<typeof describeResolvedSecret> | null;
    jobHmac: ReturnType<typeof describeResolvedSecret>;
    microsoftClient: ReturnType<typeof describeResolvedSecret> | null;
    session: ReturnType<typeof describeResolvedSecret>;
    stripeWebhook: ReturnType<typeof describeResolvedSecret>;
  };
  sessionSecretCandidates: string[];
  stripeWebhookSecretCandidates: string[];
} & ProductCapabilities;


function collectProductionSecretIssues(input: {
  parsed: z.infer<typeof apiEnvSchema>;
  secretCatalog: ApiConfig["secretCatalog"];
}) {
  const issues: string[] = [];
  const rotatingRuntimeSecrets = [
    ...input.secretCatalog.session.candidates,
    ...input.secretCatalog.jobHmac.candidates,
    input.parsed.AUTH_MFA_ENCRYPTION_KEY
  ];

  if (
    rotatingRuntimeSecrets.some(
      (secret) =>
        secret === "dev-session-secret" ||
        secret === "dev-job-hmac-secret" ||
        secret === "dev-mfa-encryption-key" ||
        hasPlaceholderMarker(secret)
    )
  ) {
    issues.push("Production secrets cannot use development defaults.");
  }

  if (hasPlaceholderMarker(input.parsed.STRIPE_SECRET_KEY)) {
    issues.push("STRIPE_SECRET_KEY cannot use placeholder values in production.");
  }

  if (
    input.secretCatalog.stripeWebhook.candidates.some((secret) =>
      hasPlaceholderMarker(secret)
    )
  ) {
  ...
  if (
    input.parsed.GOOGLE_CLIENT_SECRET &&
    hasPlaceholderMarker(input.parsed.GOOGLE_CLIENT_SECRET)
  ) {
    issues.push("GOOGLE_CLIENT_SECRET cannot use placeholder values in production.");
  }

  if (
    input.parsed.HUBSPOT_CLIENT_SECRET &&
    hasPlaceholderMarker(input.parsed.HUBSPOT_CLIENT_SECRET)
  ) {
    issues.push("HUBSPOT_CLIENT_SECRET cannot use placeholder values in production.");
  }

  if (
    input.parsed.MICROSOFT_CLIENT_SECRET &&
    hasPlaceholderMarker(input.parsed.MICROSOFT_CLIENT_SECRET)
  ) {
    issues.push("MICROSOFT_CLIENT_SECRET cannot use placeholder values in production.");
  }
...
export function getApiConfig(env: NodeJS.ProcessEnv = getEnvironmentSource()): ApiConfig {
  ...
  const secretCatalog = {
    authMfaEncryption: describeResolvedSecret({
      primary: parsed.AUTH_MFA_ENCRYPTION_KEY
    }),
    googleClient: parsed.GOOGLE_CLIENT_SECRET
      ? describeResolvedSecret({ primary: parsed.GOOGLE_CLIENT_SECRET })
      : null,
    hubspotClient: parsed.HUBSPOT_CLIENT_SECRET
      ? describeResolvedSecret({ primary: parsed.HUBSPOT_CLIENT_SECRET })
      : null,
    jobHmac: describeResolvedSecret({
      fallbacks: parsed.JOB_HMAC_GLOBAL_SECRET_FALLBACKS,
      primary: parsed.JOB_HMAC_GLOBAL_SECRET
    }),
    microsoftClient: parsed.MICROSOFT_CLIENT_SECRET
      ? describeResolvedSecret({ primary: parsed.MICROSOFT_CLIENT_SECRET })
      : null,
    session: describeResolvedSecret({
      fallbacks: parsed.SESSION_SECRET_FALLBACKS,
      primary: parsed.SESSION_SECRET
    }),
    stripeWebhook: describeResolvedSecret({
  ...
  return {
    ...
    jobHmacSecretCandidates: secretCatalog.jobHmac.candidates,
    secretCatalog,
    SENTRY_ENVIRONMENT: parsed.SENTRY_ENVIRONMENT ?? parsed.NODE_ENV,
    sessionSecretCandidates: secretCatalog.session.candidates,
    stripeWebhookSecretCandidates: secretCatalog.stripeWebhook.candidates
  };
}
