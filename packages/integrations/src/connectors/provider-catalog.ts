...
export const runtimeImplementedConnectorProviders = [
  "hubspot",
  "omie",
  "slack",
  "stripe",
...
export type RuntimeImplementedConnectorProvider =
  (typeof runtimeImplementedConnectorProviders)[number];

const runtimeImplementedConnectorProviderSet = new Set<string>(
  runtimeImplementedConnectorProviders
);

export function isRuntimeImplementedConnectorProvider(
  provider: string
): provider is RuntimeImplementedConnectorProvider {
  return runtimeImplementedConnectorProviderSet.has(provider);
}

function defineProvider(input: ConnectorProviderDefinition): ConnectorProviderDefinition {
  if (
    input.implementationStage === "implemented" &&
    !isRuntimeImplementedConnectorProvider(input.slug)
  ) {
    throw new Error(
      `Connector provider ${input.slug} cannot be implemented without a connectors-core runtime handler.`
    );
  }
...
export const connectorProviderCatalog = [
  defineProvider({
    ...
    implementationStage: "client_only",
...
export function listImplementedConnectorProviders(): ConnectorProviderDefinition[] {
  return connectorProviderCatalog
    .filter((provider) => provider.implementationStage === "implemented")
